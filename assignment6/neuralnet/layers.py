...
def l2_norm_loss(x, y):
    """
    Computes the loss and gradient for autoencoder.

    Inputs:
    - x: Predicted data, of shape (N, D) where each X[i] is a flattened array predicted
    - y: Ground Truth data, of shape (N, D) where each y[i] is a flattened array of ground truth
    ...
    """
    N = x.shape[0]
    loss = (0.5*np.sum((x - y).T.dot((x - y))))/N
    dx = (x-y)
    dx /= N
    return loss, dx
