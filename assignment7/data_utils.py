...
def load_timeseries(filename, window_size, train_test_split):
    """
        Load time series dataset
    """

    series = pd.read_csv(
        filename, sep=',', header=0, index_col=0, squeeze=True
    )
    data = series.values
    ...
    split_ratio = round(train_test_split * result.shape[0])
    ...
    return [
        ...
        x_test_raw, y_test_raw, last_raw, last
    ]
...
def normalize_windows(window_data):
    ...
    for window in window_data:
        mean = np.mean(window)
        deviation = max(window) - min(window)
        normalized_window = [((float(p) - mean) / deviation) for p in window]
